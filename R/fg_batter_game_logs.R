...
#' @export
#' @examples
#' \dontrun{fg_batter_game_logs(playerid = 6184, year = 2017)}

fg_batter_game_logs <- function(playerid, year) {
  
  url <- paste0("http://www.fangraphs.com/statsd-legacy.aspx?playerid=", playerid, "&season=", year, "&position=PB")
  ...
  payload <- payload %>%
    ...
    dplyr::rename(BB_perc = BB., K_perc = K., wRC_plus = wRC.)
  
  
  if (nrow(payload) > 1) {
    payload <- as.data.frame(sapply(payload, function(x) (gsub("\\%", "", x))), stringsAsFactors=F)
  } else {
