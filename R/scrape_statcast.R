...
#' scrape_statcast(start_date = "2016-04-06", end_date = "2016-04-06", pit_bat = 'batter')
#' }
  ...
  res <- foreach(i = 1:min(length(start_days), length(end_days))) %do%
           {
             ...
             read_csv(url, progress = F) %>% as.data.frame
           }
  processed_payload <- lapply(res, process_statcast_payload)
  return(bind_rows(processed_payload))
}

process_statcast_payload <- function(payload) {
  # Clean up formatting.
  payload <- apply(payload, 2, function(x) {
                x[x=="null"] <- NA
                x
